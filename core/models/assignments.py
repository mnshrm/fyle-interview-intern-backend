...
class Assignment(db.Model):
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        assertions.assert_valid(grade in ['A', 'B', 'C', 'D'], 'grade not present in Enum')
        assertions.assert_valid(assignment.state != AssignmentStateEnum.DRAFT,'only assignment not in draft state can be graded')
        if auth_principal.principal_id is None:
            assertions.assert_valid(assignment.teacher_id == auth_principal.teacher_id, 'Assignment was not submitted to the current teacher')
